 /*_Do Trabalho UFRN Objeto passageiro. Escopo global. Nas funcoes é acessada por window.passageiro */
...
function achouLocalArrastando(e) {		
		try{
			...
			setMotoristas(getDezMotProximos,false,STATUS.getCodOnLine());//setMotoristas(getDezMotProximos,motoristaEscolhidoDiretamente);
...
function setMotoristas(getDezMotProximos,motoristaEscolhidoDiretamente, STATUS){ //chamada em achouLocalArrastando(). Aqui já tem o preferencial baseado na distancia
	var latLng=passageiro.getLatLngI();  
	if (latLng!=null){
		 var url="../php/buscarMotoristas.php";
			 url+="?lat="+latLng.lat+"&lng="+latLng.lng+"&status="+STATUS;	
		try {   ajaxObj = new ActiveXObject("Microsoft.XMLHTTP");  }  
			catch(e) {   try {  ajaxObj = new ActiveXObject("Msxml2.XMLHTTP"); } 
			  catch(ex) { try  {     ajaxObj = new XMLHttpRequest();  
		       /************************************************************/
...
function escolheMotoristaEspecfico(){	
 ...
 var  url="../php/setaStatusComoChamando.php?codigo="+motoristaEscolhidoDiretamente.codigo+"&status="+STATUS.getCodChamDirPas(); 
  $(jm).modal('hide');
  if (passageiro.getLogin()){//so seta se estive logado
    $.get(url, function(resultado){});//Não faz nada Ainda somente setaBd.Tem que retornar ainda...
	
  }
                                        //Não passa Status do EscolhidoDiretamente
...
function verificaComoRespondeuMotorista(tempoSegundos){//Verifica Status no BD do Motorista escolhido	
try {	
var url="../php/verificaStatusMotorista.php?login="+motoristaAtual.getCodigo();

	var intervalo=setInterval(function(){
		  $.get(url, function(resultado){
			 motoristaAtual.setStatus(resultado);	            			 
				if (motoristaAtual.getStatus()==STATUS.getCodChamCorrOnLine()){//STATUS 1000011 = Chamando motorista Online normal
				   //ainda não respondeu Chamada motorista Online normal
				  
				  //clearInterval(intervalo);
				   
				  // OnChamaMotorista(STATUS.getCodChamCorrOnLine());		
				}
				if (motoristaAtual.getStatus()==STATUS.getCodOffLine()){//STATUS 0000000 = Off-line e sem tele definido em status.js
				   clearInterval(intervalo);		   
				}
				if (motoristaAtual.getStatus()==STATUS.getCodRecusou()){//VERIFICAR MELHOR STATUS   
				   clearInterval(intervalo);		   
				}
				if (motoristaAtual.getStatus()==STATUS.getCodIgnorou()){//VERIFICAR MELHOR STATUS  
				   clearInterval(intervalo);		   
				}
				if (motoristaAtual.getStatus()==STATUS.getCod()){//VERIFICAR MELHOR STATUS  
				   clearInterval(intervalo);		   
				}
...
},(app.getTempoEmSegundosVerificaStatusMotorista()||tempoSegundos));
	
} catch(er){
	alert("erro na function verificaComoRespondeuMotorista :  "+er.message);
}
