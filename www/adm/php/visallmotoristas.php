...
session_start();
 ...
 $sql="SELECT codigo, apelido,nome, lat_posicao,lng_posicao, conceito,posicao FROM app_motoristas WHERE (status='00000011' or status='00000000')  ORDER BY codigo";   
 ...
 //$sql="SELECT  apelido, lat_posicao,lng_posicao,posicao, conceito FROM app_motoristas WHERE (1)";   	
	...
	$cont2++;
	if ($cont>$cont2){
	   $matJson=$matJson.('{"nome":"'.$resultado['nome'].'","codigo":"'.$resultado['codigo'].'","apelido":"'.$resultado['apelido'].'","lat_posicao":"'.$resultado['lat_posicao'].'","lng_posicao":"'.$resultado['lng_posicao'].'"},');	 
	}else{
	   $matJson=$matJson.('{"nome":"'.$resultado['nome'].'","codigo":"'.$resultado['codigo'].'","apelido":"'.$resultado['apelido'].'","lat_posicao":"'.$resultado['lat_posicao'].'","lng_posicao":"'.$resultado['lng_posicao'].'"}');
	}
